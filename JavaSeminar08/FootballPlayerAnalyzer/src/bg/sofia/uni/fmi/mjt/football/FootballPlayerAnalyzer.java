...
public class FootballPlayerAnalyzer {
    ...
    public FootballPlayerAnalyzer(Reader reader) {
        try (var bufferedReader = new BufferedReader(reader)) {
            ...
            throw new UncheckedIOException("A problem occurred while reading the players from the file", e);
        }
    }
    ...
    public Player getHighestPaidPlayerByNationality(String nationality) {
        ...
        return players.stream()
            ...
            .orElseThrow(() -> new NoSuchElementException("No Players from this country!"));
    }
    ...
    public Map<Position, Set<Player>> groupByPosition() {
        return players.stream()
            .flatMap(pl -> pl.positions().stream()
                .map(pos -> Map.entry(pos, pl)))
    ...
    public Optional<Player> getTopProspectPlayerForPositionInBudget(Position position, long budget) {
        ...
        return players.stream()
            ...
            .max(Comparator.comparing(p -> (double) (p.overallRating() + p.potential()) / p.age()));
    }
    ...
    public Set<Player> getSimilarPlayers(Player player) {
        ...
        return Collections.unmodifiableSet(players.stream()
            .filter(p -> p.hasSamePosition(player.positions()))
            .filter(p -> p.preferredFoot().equals(player.preferredFoot()))
            .filter(p -> abs(p.overallRating() - player.overallRating()) <= maxDiff)
